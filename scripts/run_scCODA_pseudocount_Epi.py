...
for baseline in ['Non-Epithelial']: #Glia
    ...
    for n_iter in np.array([20000, 40000, 80000, 150000], dtype='int32'):
        ...
        pl.savefig(data_path + 'results_pseudocount_' + baseline + '_' + str(n_iter) + '_density.pdf')
        az.plot_trace(sim_results, var_names="beta")
        pl.savefig(data_path + 'results_pseudocount_' + baseline + '_' + str(n_iter) + '_trace.pdf')
        print(sig_effects)  
        ...
        sig_effects.to_csv(data_path + 'results_pseudocount_' + baseline + '_' + str(n_iter) + '.csv')
...
for baseline in ['Non-Epithelial']: #Glia
    ...
    for n_iter in np.array([20000, 40000, 80000, 150000], dtype='int32'):
        ...
        sig_effects = sim_results.effect_df
        az.plot_density(sim_results, var_names="beta")
        pl.savefig(data_path + 'results_pseudocount_hn_' + baseline + '_' + str(n_iter) + '_density.pdf')
        az.plot_trace(sim_results, var_names="beta")
        pl.savefig(data_path + 'results_pseudocount_hn_' + baseline + '_' + str(n_iter) + '_trace.pdf')
        print(sig_effects)
        #prepare result table to save to file
        sig_effects = sig_effects.reset_index()
        sig_effects['Health'] = sig_effects['Covariate'].astype('category')
        sig_effects['Health'] = sig_effects['Health'].cat.rename_categories({'Health[T.Inflamed]': 'Inflamed',
                                                                           'Health[T.Non-inflamed]': 'Non-inflamed'})
        sig_effects['Epithelial_test'] = sig_effects['Cell Type']
        sig_effects.to_csv(data_path + 'results_pseudocount_hn_' + baseline + '_' + str(n_iter) + '.csv')
...
for baseline in ['Non-Epithelial']: #Glia
    ...
    for n_iter in np.array([20000, 40000, 80000, 150000], dtype='int32'):
        ...
        sig_effects = sim_results.effect_df
        az.plot_density(sim_results, var_names="beta")
        pl.savefig(data_path + 'results_pseudocount_hi_' + baseline + '_' + str(n_iter) + '_density.pdf')
        az.plot_trace(sim_results, var_names="beta")
        pl.savefig(data_path + 'results_pseudocount_hi_' + baseline + '_' + str(n_iter) + '_trace.pdf')
        print(sig_effects)
        #prepare result table to save to file
        sig_effects = sig_effects.reset_index()
        sig_effects['Health'] = sig_effects['Covariate'].astype('category')
        sig_effects['Health'] = sig_effects['Health'].cat.rename_categories({'Health[T.Inflamed]': 'Inflamed',
                                                                           'Health[T.Non-inflamed]': 'Non-inflamed'})
        sig_effects['Epithelial_test'] = sig_effects['Cell Type']
        sig_effects.to_csv(data_path + 'results_pseudocount_hi_' + baseline + '_' + str(n_iter) + '.csv')
...
for baseline in ['Non-Epithelial']: #Glia
    ...
    for n_iter in np.array([20000, 40000, 80000, 150000], dtype='int32'):
        ...
        sig_effects = sim_results.effect_df
        az.plot_density(sim_results, var_names="beta")
        pl.savefig(data_path + 'results_pseudocount_ni_' + baseline + '_' + str(n_iter) + '_density.pdf')
        az.plot_trace(sim_results, var_names="beta")
        pl.savefig(data_path + 'results_pseudocount_ni_' + baseline + '_' + str(n_iter) + '_trace.pdf')
        print(sig_effects)
        #prepare result table to save to file
        sig_effects = sig_effects.reset_index()
        sig_effects['Health'] = sig_effects['Covariate'].astype('category')
        sig_effects['Health'] = sig_effects['Health'].cat.rename_categories({'Health[T.Inflamed]': 'Inflamed',
                                                                           'Health[T.Non-inflamed]': 'Non-inflamed'})
        sig_effects['Epithelial_test'] = sig_effects['Cell Type']
        sig_effects.to_csv(data_path + 'results_pseudocount_ni_' + baseline + '_' + str(n_iter) + '.csv')
